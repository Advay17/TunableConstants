        ...
        typeElement.getEnclosedElements().stream().filter(f -> f.getKind().equals(ElementKind.FIELD))
            .forEach(fieldElement -> {
              ...
              if (TUNABLE_TYPES.contains(fieldType)) {
                ...
                constructorBuilder.addStatement("$T.init" + preferencesName + "(\"" + classElement.getSimpleName() + "/"
                    + simpleName + "\"," + simpleName + ")", PREFERENCES_CLASS);
                reloadBuilder.addStatement(simpleName + " = $T.get" + preferencesName + "(\""
                    + classElement.getSimpleName() + "/" + simpleName + "\"," + simpleName + ")", PREFERENCES_CLASS);
              } else if ((elementUtil.getTypeElement("com.roboloco.tune.tunable.Tunable" + fieldType) != null
                  && typeUtil.isAssignable(
                      elementUtil.getTypeElement("com.roboloco.tune.tunable.Tunable" + fieldType).asType(),
                      elementUtil.getTypeElement("com.roboloco.tune.tunable.Tunable").asType()))
                  || (elementUtil.getTypeElement("frc.robot.util.tunable.Tunable" + fieldType) != null
                      && typeUtil.isAssignable(
                          elementUtil.getTypeElement("frc.robot.util.tunable.Tunable" + fieldType).asType(),
                          elementUtil.getTypeElement("com.roboloco.tune.tunable.Tunable").asType()))) {
                constructorBuilder.addStatement("$T tunable$S = new $T($S, $S)", ClassName.get(tuanbleConstantsPackage, simpleName));
              } else {
                return;
              }
